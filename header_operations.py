################################################################################
#                    header_operations expanded expanded v.1.0.4 (by Zion)     #
...
#                    based on header_operations for VC v.0.1.0 (by Kalarhan)   #
...
this_or_next = 0x40000000  # (this_or_next|<operation_name>, ...),
                           ...
                           # or the logic will break, as there is no next condition after the third one. This is a very common issue.
...
call_script             =    1  # (call_script, <script_id>, [<script_param>...]),
                                ...
                                # A script can fail if one of the operations in it fails and isn't enclosed in a try block or a loop
...
try_begin               =    4  # (try_begin),
                                ...
                                # and jump to whatever comes after, like a following else_try block, or (for try_end) the parent block/rest of the script.
else_try                =    5  # (else_try),
...
################################################################################
  ...
  # Mathematical operations deal with numbers.
  # The scripting and operations in the game can only deal with 32-bit signed integers.
  # Floating point numbers are emulated by the so-called
  ...
  # To avoid division by zero errors you can use (val_max, divisor, 1).
  # But if the divisor is fixed_point then add (val_max, divisor, fixed_point_multiplier).
...
gt                         = 32      # (gt, <value_a>, <value_b>),
                                     # Checks that value_a > value_b.
                                     # Succeeds if <value_a> is greater than <value_b>.
ge                         = 30      # (ge, <value_a>, <value_b>),
                                     # Checks that value_a >= value_b.
                                     # Succeeds if <value_a> is greater than or equal to <value_b>.
eq                         = 31      # (eq, <value_a>, <value_b>),
                                     # Checks that value_a == value_b.
                                     # Succeeds if <value_a> is equal to <value_b>.
neq                        = neg|eq  # (neq, <value_a>, <value_b>),
                                     # Checks that value_a != value_b.
                                     # Succeeds if <value_a> is not equal to <value_b>.
le                         = neg|gt  # (le, <value_a>, <value_b>),
                                     # Checks that value_a <= value_b.
                                     # Succeeds if <value_a> is less than or equal to <value_b>.
lt                         = neg|ge  # (lt, <value_a>, <value_b>),
                                     # Checks that value_a < value_b.
                                     # Succeeds if <value_a> is less than <value_b>.
is_between                 = 33      # (is_between, <value>, <lower_bound>, <upper_bound>),
                                     # Checks that lower_bound <= value < upper_bound.
                                     # e.g. for (is_between, reg0, 2, 5), it will succeed if reg0 was 2, 3 or 4.
                                     # This can be a bit counterintuitive, and not what you would expect, so you can express a 2, 3, 4, 5 range as (is_between, reg0, 2, 5 +1),.

# Mathematical and assignment operations

# Wherever angles are mentioned, they are to be understood as degrees, not radians.

assign                     = 2133    # (assign, <destination>, <value>),
                                     # Directly assigns a value to a variable or register.
                                     # dest = val: <destination> := <value>

store_add                  = 2120    # (store_add, <destination>, <value_a>, <value_b>),
                                     # Addition, dest = val_a + val_b.
                                     # Assigns <destination> := <value_a> + <value_b>
store_sub                  = 2121    # (store_sub, <destination>, <value_a>, <value_b>),
                                     # Subtraction, dest = val_a - val_b.
                                     # Assigns <destination> := <value_a> - <value_b>
store_mul                  = 2122    # (store_mul, <destination>, <value_a>, <value_b>),
                                     # Multiplication, dest = val_a * val_b.
                                     # Assigns <destination> := <value_a> * <value_b>
store_div                  = 2123    # (store_div, <destination>, <value_a>, <value_b>),
                                     # Integer division, dest = val_a / val_b.
                                     # Assigns <destination> := <value_a> / <value_b>
                                     # Fractional part will be truncated. Negative values will be rounded up.
store_mod                  = 2119    # (store_mod, <destination>, <value_a>, <value_b>),
                                     # Modulo, dest = val_a % val_b.
                                     # Assigns <destination> := <value_a> MOD <value_b>

val_add                    = 2105    # (val_add, <destination>, <value>),
                                     # Addition, dest += val, self-increments.
                                     # Assigns <destination> := <destination> + <value>
val_sub                    = 2106    # (val_sub, <destination>, <value>),
                                     # Subtraction, dest -= val, self-decrements.
                                     # Assigns <destination> := <destination> - <value>
val_mul                    = 2107    # (val_mul, <destination>, <value>),
                                     # Multiplication, dest *= val.
                                     # Assigns <destination> := <destination> * <value>
val_div                    = 2108    # (val_div, <destination>, <value>),
                                     # Integer division, dest /= val.
                                     # Assigns <destination> := <destination> / <value>
                                     # Fractional part will be truncated. Negative values will be rounded up.
val_mod                    = 2109    # (val_mod, <destination>, <value>),
                                     # Modulo operation, dest %= val.
                                     # gets the remainder of integer division by <value>.
                                     # can be used to wrap numbers so that they loop around the {0, <value>} range.
...
val_clamp                  = 2112    # (val_clamp, <destination>, <lower_bound>, <upper_bound>),
                                     ...
                                     # we would do (val_clamp, ":i", 0, 10 +1), constraining that -4 into a 0,
...
store_sqrt                 = 2125    # (store_sqrt, <destination_fixed_point>, <value_fixed_point>),
                                     ...
                                     # Result will be rounded up (to the biggest integer value).
...
store_sin                  = 2127    # (store_sin, <destination_fixed_point>, <value_fixed_point>),
                                     ...
                                     # Sinus of <value> in degrees represented as fixed point float.
                                     # return value is also expressed as fixed point.
                                     # Example: when fixed_point = 1000, to calculate sin(90°)=1 you should pass 90*1000 as parameter,
                                     # and the result will be 1*1000.
store_cos                  = 2128    # (store_cos, <destination_fixed_point>, <value_fixed_point>),
                                     # Assigns dest := COS (value)
                                     # Cosinus of <value> in degrees represented as fixed point float.
                                     # return value is also expressed as fixed point.
                                     # Example: when fixed_point = 1000, to calculate cos(180°)=-1 you should pass 180*1000 as parameter,
                                     # and the result will be -1*1000.
store_tan                  = 2129    # (store_tan, <destination_fixed_point>, <value_fixed_point>),
                                     # Assigns dest := TAN (value)
                                     # Tangens of <value> in degrees represented as fixed point float.
                                     # return value is also expressed as fixed point.
                                     # Example: when fixed_point = 1000, to calculate tan(45°)=1 you should pass 45*1000 as parameter,
                                     # and the result will be 1*1000.
store_asin                 = 2140    # (store_asin, <destination_fixed_point>, <value_fixed_point>),
                                     # Assigns dest := ARCSIN (value)
                                     # Arcus sinus (sin⁻¹) is the inverse of sinus: while sinus takes an angle and returns a real number,
                                     # arcus sinus takes a real number and returns the smallest angle for which sinus would produce such a number.
                                     # Input is a fixed point value, and the returned angle (in degrees) is also fixed point.
                                     # Example: when fixed_point = 1000, to calculate asin(1)=90° you should pass 1*1000 as parameter,
                                     # and the result will be 90*1000.
store_acos                 = 2141    # (store_acos, <destination_fixed_point>, <value_fixed_point>),
                                     # Assigns dest := ARCCOS (value)
                                     # Arcus cosinus (cos⁻¹) is the inverse of cosinus: while cosinus takes an angle and returns a real number,
                                     # arcus cosinus takes a real number and returns the smallest angle for which cosinus would produce such a number.
                                     # Input is a fixed point value, and the returned angle (in degrees) is also fixed point.
                                     # Example: when fixed_point = 1000, to calculate acos(-1)=180° you should pass (-1)*1000 as parameter,
                                     # and the result will be 180*1000.
store_atan                 = 2142    # (store_atan, <destination_fixed_point>, <value_fixed_point>),
                                     # Assigns dest := ARCTAN (value)
                                     # Arcus tangens (tan⁻¹) is the inverse of tangens: while tangens takes an angle and returns a real number,
                                     # arcus tangens takes a real number and returns the smallest angle for which tangens would produce such a number.
                                     # Input is a fixed point value, and the returned angle (in degrees) is also fixed point.
                                     # Example: when fixed_point = 1000, to calculate atan(1)=45° you should pass 1*1000 as parameter,
                                     # and the result will be 45*1000.
...
store_random               = 2135    # (store_random, <destination>, <upper_range>),
                                     # Stores a random value in the range of 0..<upper_range>-1.
...
store_random_in_range      = 2136    # (store_random_in_range, <destination>, <range_low>, <range_high>),
                                     ...
                                     # Returned value is limited to 32767 (0x7FFF).
                                     # E.g. (store_random_in_range, random, 10, 0xFFFF) yields values from 10 to (0x7FFF + 10).
shuffle_range              = 2134    # (shuffle_range, <reg_no>, <upper_bound>),
                                     # Randomly shuffles a range of registers from <reg_no> up to <upper_bound - 1>,
                                     # reordering the values contained in them. Commonly used for list randomization.
...
convert_to_fixed_point     = 2130    # (convert_to_fixed_point, <destination_fixed_point>),
                                     ...
                                     # Note: As shown by Erundil here [https://forums.taleworlds.com/index.php?threads/python-script-scheme-exchange.8652/page-40#post-9897421],
                                     # using 1 as the parameter effectively removes the need for a (get_fixed_point_multiplier, <destination>),
                                     # operation and gives you the same thing, if used like this:
                                     ######
                                     # (assign, ":current_fixed_point_mult", 1),
                                     # (convert_to_fixed_point, ":current_fixed_point_mult"),
                                     ######
...
store_script_param_1        =   21  # (store_script_param_1, <destination>),
                                    ...
                                    # If the script was called without this parameter being specified, this operation assigns a zero.
...
store_script_param          =   23  # (store_script_param, <destination>, <script_param_index>),
                                    # Retrieve the value of arbitrary script parameter (generally used when script accepts more than two).
                                    # Parameters are enumerated starting from 1.
                                    # Maximum are 15 parameters (16 operands maximum - 1 operand with script name).
                                    # If the script was called without this parameter being specified, this operation assigns a zero.
...
store_trigger_param_1       = 2071  # (store_trigger_param_1, <destination>),
                                    # Retrieve the value of the first trigger parameter.
                                    # Will retrieve trigger's parameters even when called from inside a script,
                                    # for as long as that script is running within trigger context.
store_trigger_param_2       = 2072  # (store_trigger_param_2, <destination>),
                                    # Retrieve the value of the second trigger parameter.
                                    # Will retrieve trigger's parameters even when called from inside a script,
                                    # for as long as that script is running within trigger context.
store_trigger_param_3       = 2073  # (store_trigger_param_3, <destination>),
                                    # Retrieve the value of the third trigger parameter.
                                    # Will retrieve trigger's parameters even when called from inside a script,
                                    # for as long as that script is running within trigger context.
store_trigger_param         = 2070  # (store_trigger_param, <destination>, <trigger_param_no>),
                                    # Version 1.153+.
                                    # Retrieve the value of arbitrary trigger parameter.
                                    # Parameters are enumerated starting from 1.
                                    # Will retrieve trigger's parameters even when called from inside a script,
                                    # for as long as that script is running within trigger context.
...
key_is_down                     = 70  # (key_is_down, <key_code>),
                                      # Checks that the specified key is currently pressed (in the current frame).
                                      # See header_triggers.py for key code reference.
key_clicked                     = 71  # (key_clicked, <key_code>),
                                      # Checks that the specified key has just been pressed (current frame and wasn't clicked in the previous frame).
                                      # See header_triggers.py for key code reference.
                                      # Is only true in the one frame where the engine first recognizes that the key is pressed.
                                      # Will work reliably only on mission template triggers that happen on every frame (i.e. check interval 0)
                                      # as well as on simple_triggers or old form triggers that are check interval 0 too.
game_key_is_down                = 72  # (game_key_is_down, <game_key_code>),
                                      # Checks that the specified game key is currently pressed (ignoring if it was clicked in the previous frame).
                                      # Is true in every frame that the key is down.
                                      # See header_triggers.py for game key code reference.
...
omit_key_once                   = 77  # (omit_key_once, <key_code>),
                                      # Forces the game to ignore default bound action for the specified game key on the current game frame.
...
map_free                   =   37  # (map_free),
                                   # Checks that the player is currently on the global map and can move freely:
                                   # no game screens/pop-ups are open, player isn't resting, and isn't in captivity of another party
                                   # (not fully verified for rest_for_hours_interactive). 4research
...
# These operations only work when the Steamworks library is running and the player is logged-in. Otherwise they fail early and return -1.
# They will also be disabled if cheats are toggled on via launcher. And, as expected, they won't do anything in the standalone and GOG versions of the game.
# This achievement data is not stored locally or in the save either, everything is done or retrieved through the Steam servers.
...
party_collect_attachments_to_party    = 1662  # (party_collect_attachments_to_party, <source_party_id>, <aggregated_party_id>),
                                              # Mostly used in various battle and AI calculations.
                                              # Creates an aggregated party from <source_party_id> and all parties attached to it.
                                              # 4research: does it move troops from original parties into the aggregate party or does it copy them? There is no dedicated operation for copying parties, so this could be useful.
...
store_random_party_in_range           = 2254  # (store_random_party_in_range, <destination>, <lower_bound>, <upper_bound>),
                                              # Retrieves one random party from the range given. Generally used only for predefined parties (towns, villages etc).
...
store_distance_to_party_from_party    = 2281  # (store_distance_to_party_from_party, <destination>, <party_id_1>, <party_id_2>),
                                              ...
                                              # Returns the same values as get_distance_between_positions_in_meters (which on the world map is not meters - see comment to get_distance_between_positions_in_meters),
                                              # but doesn't require us to manually store positions of the parties (so it's basically just get_distance_between_parties).
...
party_set_next_battle_simulation_time = 1667  # (party_set_next_battle_simulation_time, <party_id>, <next_simulation_time_in_hours>),
                                              # Defines the period of time (in hours) after which the battle must be simulated for the specified party for the next time.
...
party_set_marshall                    = 1604  # (party_set_marshall, <party_id>, <value>),
                                              # Sets party as a marshall party or turns it back to a normal party. Value is either 1 or 0.
                                              # This affects party behavior, but exact effects are not known.
party_set_marshal = party_set_marshall        # (party_set_marshal, <party_id>, <value>),
                                              # Alternative operation name spelling for compatibility with Viking Conquest DLC module system.
party_set_flags                       = 1603  # (party_set_flags, <party_id>, <flag>, <clear_or_set>),
                                              ...
                                              # See the bitwise operations section in this document to learn about how to toggle individual bits.
                                              ...
                                              # Keep in mind that there is no matching operation to get the flags natively (e.g. party_get_flags), so one would need to emulate it by
                                              # automatically saving their initial known new-game state at compile time into a slot, and then using a custom wrapper function that calls
                                              # this operation and also updates the slot contents to keep them in sync. That way you can retrieve the current party flag data from the slot as a substitute.
                                              # Swyter's explanation: [https://forums.taleworlds.com/index.php?threads/python-script-scheme-exchange.8652/page-41#post-9905622]
                                              # Example by Erundil: [https://forums.taleworlds.com/index.php?threads/python-script-scheme-exchange.8652/post-9912783]
...
troop_get_class                          = 1516  # (troop_get_class, <destination>, <troop_id>),
                                                 ...
                                                 #  - if troop has tf_mounted flag, class = 2 (Cavalry)
                                                 #  - otherwise if troop has tf_guarantee_ranged flag, class = 1 (Archers)
                                                 #  - otherwise class = 0 (Infantry)
...
add_gold_as_xp                           = 1063  # (add_gold_as_xp, <value>, [troop_id]),
                                                 ...
                                                 # Conversion rate is unclear and apparently somewhat randomized (three runs with 1000 gold produced values 1091, 804 and 799).
...
troop_add_gold                           = 1528  # (troop_add_gold, <troop_id>, <value>),
                                                 ...
                                                 # If troop is trp_player and value is non-zero, plays snd_money_received and displays message ui_you_got_value ("You got %s.").
                                                 # Note: this popup/SFX behavior occurs regardless of (set_player_troop).
troop_remove_gold                        = 1529  # (troop_remove_gold, <troop_id>, <value>),
                                                 # Removes gold from troop. Generally used with player or hero troops.
                                                 # If troop is trp_player and value is non-zero, plays snd_money_paid and displays message ui_you_lost_value ("You lost %s.").
                                                 # Using a negative value cannot be used to silently add money to the player.
...
# Each returned morph key value has three bits, so the slider value always fits into the 0..7 range.
...
check_quest_finished          =  201  # (check_quest_finished, <quest_id>),
                                      ...
                                      # Internally it only checks against the finished flag in the quest's progression field, see below.
check_quest_succeeded         =  202  # (check_quest_succeeded, <quest_id>),
                                      ...
                                      # Internally it only checks against the succeeded flag in the quest's progression field, see below.
check_quest_failed            =  203  # (check_quest_failed, <quest_id>),
                                      ...
                                      # Internally it only checks against the failed flag in the quest's progression field, see below.
check_quest_concluded         =  204  # (check_quest_concluded, <quest_id>),
                                      ...
                                      # Internally it only checks against the concluded flag in the quest's progression field, see below.
...
start_quest                   = 1280  # (start_quest, <quest_id>, <giver_troop_id>),
                                      ...
                                      # Clears any previous quest state, then toggles on the active flag and shows a ui_new_quest_taken message.
conclude_quest                = 1286  # (conclude_quest, <quest_id>),
                                      ...
                                      # Only toggles on the concluded flag and shows a ui_quest_concluded_value message.
...
cancel_quest                  = 1284  # (cancel_quest, <quest_id>),
                                      # Cancels specified quest without completing it. Generally used to remove it from the list of active quests.
...
set_quest_progression         = 1285  # (set_quest_progression, <quest_id>, <value>),
                                      ...
                                      # In the save storage there is a progression field [https://github.com/Swyter/mab-tools/blob/df0c08f0d15b3f3a1e0e2215278598c14790c8e5/mab_sav_file.bt#L113] for each quest,
                                      # and setting it to the maximum and minimum values only affects/controls the bottom-most eight bits, with the top bits being used as a bitfield with flags to hold the current quest states.
                                      

                                      # 0001 0111 1111 1111 (6143) <- after setting the <value> to 0xfffffffff.
                                      # 0001 0111 0000 0000 (5888) <- after setting the <value> to 0.
                                      #           ^^^^ ^^^^
                                      # 0000 0001 0000 0000 ( 256) <- after (start_quest), the engine marks our quest as active
                                      # 0001 0101 0000 0000 (5376) <- after (start_quest), (succeed_quest), the engine marks our quest as active, succeeded and concluded
                                      # 0001 1101 0000 0000 (7424) <- after (start_quest), (succeed_quest), (fail_quest),
                                      # 0001 1001 0000 0000 (6400) <- after (start_quest), (fail_quest),
                                      # 0001 0001 0000 0000 (4352) <- after (start_quest), (conclude_quest),
                                      # 0001 0011 0000 0000 (4864) <- after (complete_quest), marks it as finished when quest is unflagged (0)
                                      # 0000 0000 0000 0000 (   0) <- after (complete_quest), blanks it out when it has qf_random_quest
                                      # 0000 0000 0000 0000 (   0) <- after (cancel_quest), blanks it out
                                      # ^^^^ ^^^^
                                      #
                                      # So after all the tests, the bit flag meanings seem to be as follows:
                                      # 0000 0001 0000 0000 quest is active
                                      # 0000 0010 0000 0000 quest is finished (for when qf_random_quest is missing)
                                      # 0000 0100 0000 0000 quest has succeeded
                                      # 0000 1000 0000 0000 quest has failed
                                      # 0001 0000 0000 0000 quest has concluded (often paired w/ fail/succeed or both)
                                      #
                                      # Takeaways:
                                      # - A quest can be seemingly be flagged/marked as both failed and succeeded at the same time; calling (fail_quest) does not clear the succeed flag and vice versa. So bugs may happen, its status depends on the order the dialog checks the flags.
                                      # - The only difference between (complete_quest), and (cancel_quest), seems to be the different user-facing log messages. Both blank the state.
                                      # - It seems like what controls quest visibility in the list is having notes or not. The actual quest state isn't exposed or taken into account in the UI.
                                      # - The top and bottom halves of the progression save data field work independently; one can clear the quest state while still having random data in the bottom part no matter the state and vice versa.
...
stop_sound_channel       =  616  # (stop_sound_channel, <sound_channel_no>),
                                 ...
                                 # Used to fix bug with looping sounds in Viking Conquest.
...
tutorial_message_set_size           = 1124  # (tutorial_message_set_size, <size_x>, <size_y>), 
                                            # Defines the font size of the tutorial box. Assumes screen size is 1000*750.
                                            # The size of the tutorial box is always the same, depending how much text it has (one can see it with background on 1, it has always full width).
...
# The following operations let us define custom shader uniforms (i.e. global shader variables), named after the string in <tt><parameter_name></tt>;
...
agent_get_group                          = 1765  # (agent_get_group, <destination>, <agent_id>),
                                                 ...
                                                 # In singleplayer: Retrieves agent group which is the same as agent team.
...
# position_aim_at_position   # (position_point_towards, <position_reg>, <aim_position_reg>),
                             # Rotates the position so its Y axis points at specified aim position.
...
# ui_create_item            # (ui_create_item, <destination>, <item>, <x>, <y>, [<scale_x>], [<scale_y>]),
                            # If only <scale_x> is provided, it will be used for both dimensions.
# ui_create_horslider       # (ui_create_numberbox, <destination>, <x>, <y>, <min_value>, <max_value>, [<scale_x>], [<scale_y>]),
...
key_is_down                  =   70  # (key_is_down, <key>, [<bypass_console_check>]),
                                     # Fails if <key> is not currently down
key_clicked                  =   71  # (key_clicked, <key>, [<bypass_console_check>]),
                                     # Fails if <key> is not clicked on the specific frame
game_key_is_down             =   72  # (game_key_is_down, <game_key_no>, [<bypass_console_check>]),
                                     # Fails if <game_key_no> is not currently down
game_key_clicked             =   73  # (game_key_clicked, <game_key_no>, [<bypass_console_check>]),
                                     # Fails if <game_key_no> is not clicked on the specific frame
server_set_max_num_players   =  491  # (server_set_max_num_players, <max_players>, [<max_private_players>]),
                                     # Sets maximum players to <max_players> and maximum private players to [<max_private_players>] (default = same as <max_players>).
                                     # Both values must be in the range 2-250, [<max_private_players>] can't be lower than <max_players>
position_rotate_x            =  723  # (position_rotate_x, <position_register>, <angle>, [<use_global_axis>]),
                                     # Rotates <position_register> around the x-axis by <angle> degrees
position_rotate_y            =  724  # (position_rotate_y, <position_register>, <angle>, [<use_global_axis>]),
                                     # Rotates <position_register> around the y-axis by <angle> degrees
position_rotate_z            =  725  # (position_rotate_z, <position_register>, <angle>, [<use_global_axis>]),
                                     # Rotates <position_register> around the z-axis by <angle> degrees
position_rotate_z_floating   =  734  # (position_rotate_z_floating, <position_register>, <angle_fixed_point>, [<use_global_axis>]),
                                     # Rotates <position_register> around the z-axis by <angle_fixed_point> degrees
position_rotate_x_floating   =  738  # (position_rotate_x_floating, <position_register>, <angle_fixed_point>, [<use_global_axis>]),
                                     # Rotates <position_register> around the x-axis by <angle_fixed_point> degrees
position_rotate_y_floating   =  739  # (position_rotate_y_floating, <position_register>, <angle_fixed_point>, [<use_global_axis>]),
                                     # Rotates <position_register> around the y-axis by <angle_fixed_point> degrees
is_vanilla_warband           = 1004  # (is_vanilla_warband),
...
start_map_conversation       = 1025  # (start_map_conversation, <troop_id>, [troop_dna], [set_dialog_state], [dialog_state], [from_presentation]),
                                     # Starts a conversation with the selected <troop_id>. Can be called directly from global map or game menus.
                                     ...
                                     # [<from_presentation>] - can be started from presentation and returns to that presentation after dialog close.
agent_set_animation_progress = 1743  # (agent_set_animation_progress, <agent_no>, <value_fixed_point>, [<channel_no>]),
                                     # Sets <agent_no>'s channel [<channel_no>] animation progress to <value_fixed_point>.
prop_instance_receive_damage = 1877  # (prop_instance_receive_damage, <prop_instance_no>, <agent_no>, <damage>, [<advanced>]),
                                     # <prop_instance_no> received <damage> damage from <agent_no>.
                                     # If [<advanced>] is non-zero ti_on_scene_prop_hit will be called and the damage dealt will be sent to clients.
add_point_light              = 1960  # (add_point_light, [<flicker_magnitude>], [<flicker_interval>], [<range>]),
                                     # Adds a point light with [<flicker_magnitude>] and [<flicker_interval>] ([<range>] if set - in meters).
add_point_light_to_entity    = 1961  # (add_point_light_to_entity, [<flicker_magnitude>], [<flicker_interval>], [<range>]),
                                     # Adds a point light to entity with [<flicker_magnitude>] and [<flicker_interval>] ([<range>] if set - in meters).

val_shr    = 2800  # (val_shr, <value>, <shift>),
                   # Performs an arithmetic right bit shift by <shift> on <value>
store_shr  = 2801  # (store_shr, <destination>, <value>, <shift>),
                   # Performs an arithmetic right bit shift by <shift> on <value> and stores the result into <destination>
val_lshr   = 2802  # (val_lshr, <value>, <shift>),
                   # Performs a logical right bit shift by <shift> on <value>
store_lshr = 2803  # (store_lshr, <destination>, <value>, <shift>),
                   # Performs a logical right bit shift by <shift> on <value> and stores the result into <destination>
val_shl    = 2804  # (val_shl, <value>, <shift>),
                   # Performs a left bit shift by <shift> on <value>
store_shl  = 2805  # (store_shl, <destination>, <value>, <shift>),
                   # Performs a left bit shift by <shift> on <value> and stores the result into <destination>
val_xor    = 2806  # (val_xor, <value1>, <value2>),
                   # Performs a bitwise exclusive or between <value1> and <value2>
                   # 0 xor 1 = 1;   1 xor 1 = 0
store_xor  = 2807  # (store_xor, <destination>, <value1>, <value2>),
                   # Performs a bitwise exclusive or between <value1> and <value2> and stores the result into <destination>
val_not    = 2808  # (val_not, <value>),
                   # Performs a bitwise complement on <value>
                   # not(0) = -1;   not(-1) = 0
store_not  = 2809  # (store_not, <destination>, <value>),
                   # Performs a bitwise complement on <value> and stores the result into <destination>

player_set_skin               = 2900  # (player_set_skin, <player_no>, <skin_no>),
                                      # Sets <player_no>'s skin (gender) to <skin_no> (requires, for WSE: network_compatible = 0 in wse_settings.ini, for WSE2: bBreakWarbandCompatibility=true in rgl_config.ini)
player_stop_controlling_agent = 2901  # (player_stop_controlling_agent, <player_no>),
                                      # Gives <player_no>'s agent back to AI control (requires, for WSE: network_compatible = 0 in wse_settings.ini, for WSE2: bBreakWarbandCompatibility=true in rgl_config.ini)
player_set_banner_id          = 2902  # (player_set_banner_id, <player_no>, <banner_no>),
                                      # Sets <player_no>'s banner to <banner_no>
player_set_username           = 2903  # (player_set_username, <player_no>, <string_no>),
                                      # Sets <player_no>'s username to <string_no>
...
register_set                           = 3001  # (register_set, <index>, <value>),
                                               # Sets the value of register <index> to <value>
store_wse_version                      = 3002  # (store_wse_version, <destination>, <component>),
                                               # Stores <component> of the WSE version (0: major, 1: minor, 2: build) version into <destination>
item_slot_gt                           = 3003  # (item_slot_gt, <item_kind_no>, <slot_no>, <value>),
                                               # Fails if <item_kind_no>'s <slot_no> is not greater than <value>
party_template_slot_gt                 = 3004  # (party_template_slot_gt, <party_template_no>, <slot_no>, <value>),
                                               # Fails if <party_template_no>'s <slot_no> is not greater than <value>
troop_slot_gt                          = 3005  # (troop_slot_gt, <troop_no>, <slot_no>, <value>),
                                               # Fails if <troop_no>'s <slot_no> is not greater than <value>
faction_slot_gt                        = 3006  # (faction_slot_gt, <faction_no>, <slot_no>, <value>),
                                               # Fails if <faction_no>'s <slot_no> is not greater than <value>
quest_slot_gt                          = 3007  # (quest_slot_gt, <quest_no>, <slot_no>, <value>),
                                               # Fails if <quest_no>'s <slot_no> is not greater than <value>
scene_slot_gt                          = 3008  # (scene_slot_gt, <site_no>, <slot_no>, <value>),
                                               # Fails if <site_no>'s <slot_no> is not greater than <value>
party_slot_gt                          = 3009  # (party_slot_gt, <party_no>, <slot_no>, <value>),
                                               # Fails if <party_no>'s <slot_no> is not greater than <value>
player_slot_gt                         = 3010  # (player_slot_gt, <player_no>, <slot_no>, <value>),
                                               # Fails if <player_no>'s <slot_no> is not greater than <value>
team_slot_gt                           = 3011  # (team_slot_gt, <team_no>, <slot_no>, <value>),
                                               # Fails if <team_no>'s <slot_no> is not greater than <value>
agent_slot_gt                          = 3012  # (agent_slot_gt, <agent_no>, <slot_no>, <value>),
                                               # Fails if <agent_no>'s <slot_no> is not greater than <value>
scene_prop_slot_gt                     = 3013  # (scene_prop_slot_gt, <prop_instance_no>, <slot_no>, <value>),
                                               # Fails if <prop_instance_no>'s <slot_no> is not greater than <value>
store_current_trigger                  = 3014  # (store_current_trigger, <destination>),
                                               # Stores the current trigger into <destination> (0 if not in a trigger)
return_values                          = 3015  # (return_values, [<value_1>], [<value_2>], [<value_3>], [<value_4>], [<value_5>], [<value_6>], [<value_7>], [<value_8>], [<value_9>], [<value_10>], [<value_11>], [<value_12>], [<value_13>], [<value_14>], [<value_15>], [<value_16>]),
                                               # Stores up to 16 return values
store_num_return_values                = 3016  # (store_num_return_values, <destination>),
                                               # Stores the amount of return values available into <destination>
store_return_value                     = 3017  # (store_return_value, <destination>, [<value>]),
                                               # Stores return value no. [<value>] into <destination>
...
mtrand                                 = 3021  # (mtrand, <destination>, <min>, <max>),
                                               # Stores a random value between <min> and <max> into <destination> using the MT19937 random generator
get_time                               = 3022  # (get_time, <destination>, [<local>]),
                                               # Stores the current UNIX time into <destination>. If [<local>] is non-zero, it will store local time instead of universal time.
order_flag_is_active                   = 3023  # (order_flag_is_active),
                                               # Fails if the order flag is not being placed
play_bink_file                         = 3024  # (play_bink_file, <path_from_module_directory>, [<duration_in_ms>]),
                                               # Plays a .bik file located at <path_from_module_directory>. If [<duration_in_ms>] is not set the full movie will be played
process_advanced_url_messages          = 3025  # (process_advanced_url_messages),
                                               # Forces processing of URL messages sent with send_message_to_url_advanced
sleep_ms                               = 3026  # (sleep_ms, <time>),
                                               # Sleeps (blocking the game) for <time> ms
...
timer_get_elapsed_time                 = 3028  # (timer_get_elapsed_time, <destination>, <timer_register_no>),
                                               # Stores <timer_register_no>'s elapsed time into <destination>
shell_open_url                         = 3029  # (shell_open_url, <url>),
                                               # Opens <url> in default browser. Support only http://, https://, ftp:// and ts3server:// urls.
set_main_party                         = 3030  # (set_main_party, <party_no>),
                                               # Sets player's main party to <party_no>. Dynamic spawned party (not listed in module_parties.py) will corrupt the savegame!
...
store_application_time                 = 3035  # (store_application_time, <destination>),
                                               # Stores application time into <destination> in milliseconds
is_party_skill                         = 3036  # (is_party_skill, <skill_no>),
                                               # Fails if <skill_no> is not effects party
get_campaign_time                      = 3037  # (get_campaign_time, <destination>),
                                               # Stores campaign time into <destination>. 100000 = 1 game hour
set_campaign_time                      = 3038  # (set_campaign_time, <value>),
                                               # Sets campaign time to <value>. 100000 = 1 game hour
get_mouse_map_coordinates              = 3039  # (get_mouse_map_coordinates, <position_register>),
                                               # Stores mouse map coordinates into <position_register> (requires WSE2)
...
key_released      = 3101  # (key_released, <key>, [<bypass_console_check>]),
                          # Fails if <key> wasn't released in the current frame
game_key_released = 3102  # (game_key_released, <game_key_no>, [<bypass_console_check>]),
                          # Fails if <game_key_no> wasn't released in the current frame

dict_create                = 3200  # (dict_create, <destination>),
                                   # Creates an empty dictionary object and stores it into <destination>
...
dict_is_empty              = 3206  # (dict_is_empty, <dict>),
                                   # Fails if <dict> is not empty
dict_has_key               = 3207  # (dict_has_key, <dict>, <key>),
                                   # Fails if <key> is not present in <dict>
dict_get_size              = 3208  # (dict_get_size, <destination>, <dict>),
                                   # Stores the count of key-value pairs in <dict> into <destination>
...
dict_set_int               = 3213  # (dict_set_int, <dict>, <key>, <value>),
                                   # Adds (or changes) <value> as the numeric value paired to <key>
...
dict_load_file_json        = 3217  # (dict_load_file_json, <dict>, <file>, [<mode>]),
                                   # Loads a dictionary json file into <dict>. [<mode>]: see above
dict_save_json             = 3218  # (dict_save_json, <dict>, <file>),
                                   # Saves <dict> into a json file. For security reasons, <file> is just a name, not a full path, and will be stored into a WSE managed directory
dict_from_url_encoded_json = 3219  # (dict_from_url_encoded_json, <dict>, <string>, [<mode>]),
                                   # Loads a url encoded json <string> into <dict>. [<mode>]: see above
dict_to_url_encoded_json   = 3220  # (dict_to_url_encoded_json, <string_register>, <dict>),
                                   # Saves <dict> into a url encoded json and stores into <string_register>
dict_erase                 = 3221  # (dict_erase, <dict>, <key>),
                                   # Removes value from <dict> paired to <key>
dict_delete_file_json      = 3222  # (dict_delete_file_json, <file>),
                                   # Deletes dictionary json file <file> from disk

agent_get_item_modifier                          = 3300  # (agent_get_item_modifier, <destination>, <agent_no>),
                                                         # Stores <agent_no>'s horse item modifier (-1 if agent is not a horse) into <destination>
agent_get_item_slot_modifier                     = 3301  # (agent_get_item_slot_modifier, <destination>, <agent_no>, <item_slot_no>),
                                                         # Stores <agent_no>'s <item_slot_no> modifier into <destination>
agent_get_animation_progress                     = 3302  # (agent_get_animation_progress, <destination>, <agent_no>, [<channel_no>]),
                                                         # Stores <agent_no>'s channel [<channel_no>] animation progress (in %%) into <destination>
agent_get_dna                                    = 3303  # (agent_get_dna, <destination>, <agent_no>),
                                                         # Stores <agent_no>'s dna into <destination>
agent_get_ground_scene_prop                      = 3304  # (agent_get_ground_scene_prop, <destination>, <agent_no>),
                                                         # Stores the prop instance on which <agent_no> is standing into <destination>
agent_set_item_slot_ammo                         = 3305  # (agent_set_item_slot_ammo, <agent_no>, <item_slot_no>, <value>),
                                                         # Sets <agent_no>'s <item_slot_no> ammo count to <value>
agent_get_item_slot_hit_points                   = 3306  # (agent_get_item_slot_hit_points, <destination>, <agent_no>, <item_slot_no>),
                                                         # Stores <agent_no>'s <item_slot_no> shield hitpoints into <destination>
agent_set_item_slot_hit_points                   = 3307  # (agent_set_item_slot_hit_points, <agent_no>, <item_slot_no>, <value>),
                                                         # Sets <agent_no>'s <item_slot_no> shield hitpoints to <value>
agent_get_wielded_item_slot_no                   = 3308  # (agent_get_wielded_item_slot_no, <destination>, <agent_no>, [<hand_no>]),
                                                         # Stores <agent_no>'s wielded item slot for [<hand_no>] into <destination>
agent_get_scale                                  = 3309  # (agent_get_scale, <destination_fixed_point>, <agent_no>),
                                                         # Stores <agent_no>'s scale into <destination_fixed_point>
agent_set_forced_lod                             = 3310  # (agent_set_forced_lod, <agent_no>, <lod_level>),
                                                         # Forces <agent_no>'s LOD level to <lod_level> (0 = auto)
agent_get_item_slot_flags                        = 3311  # (agent_get_item_slot_flags, <destination>, <agent_no>, <item_slot_no>),
                                                         # Stores <agent_no>'s <item_slot_no> item slot flags into <destination>
agent_ai_get_move_target_position                = 3312  # (agent_ai_get_move_target_position, <position_register>, <agent_no>),
                                                         # Stores <agent_no>'s move target position agent into <position_register>
...
agent_body_meta_mesh_set_vertex_keys_time_point  = 3317  # (agent_body_meta_mesh_set_vertex_keys_time_point, <agent_no>, <body_meta_mesh>, <time_point>),
                                                         # Sets <agent_no>'s <body_meta_mesh> vertex keys time point to <time_point>
agent_body_meta_mesh_set_visibility              = 3318  # (agent_body_meta_mesh_set_visibility, <agent_no>, <body_meta_mesh>, <value>),
                                                         # Shows (<value> = 1) or hides (<value> = 0) <agent_no>'s <body_meta_mesh>
agent_set_personal_animation                     = 3319  # (agent_set_personal_animation, <agent_no>, <anim_no>, <anim_no>),
                                                         # Replaces <agent_no>'s default <anim_no> to personal <anim_no>
agent_get_personal_animation                     = 3320  # (agent_get_personal_animation, <destination>, <agent_no>, <anim_no>),
                                                         # Stores <agent_no>'s personal <anim_no> into <destination>
agent_set_default_animations                     = 3321  # (agent_set_default_animations, <agent_no>),
                                                         # Removes <agent_no>'s personal animations
agent_cancel_current_animation                   = 3322  # (agent_cancel_current_animation, <agent_no>, <channel_no>),
                                                         # Cancels <agent_no>'s channel <2> animation
agent_get_ranged_damage_modifier                 = 3323  # (agent_get_ranged_damage_modifier, <destination>, <agent_no>),
                                                         # Stores <agent_no>'s ranged damage modifier into <destination>
agent_add_stun                                   = 3324  # (agent_add_stun, <agent_no>, <duration>),
                                                         # Adds stun to <agent_no> for <duration> milliseconds
agent_body_meta_mesh_deform_in_range             = 3325  # (agent_body_meta_mesh_deform_in_range, <agent_no>, <body_meta_mesh>, <start_frame>, <end_frame>, <time_period>),
                                                         # Animates <agent_no>'s <body_meta_mesh> from <start_frame> to <end_frame> within the specified <time_period> (in milliseconds)
...
agent_get_current_vertical_speed                 = 3331  # (agent_get_current_vertical_speed, <destination>, <agent_no>),
                                                         # Stores <agent_no>'s current vertical speed into <destination> (in centimeters per second)
agent_set_current_vertical_speed                 = 3332  # (agent_set_current_vertical_speed, <agent_no>, <value>),
                                                         # Sets <agent_no>'s current vertical speed to <value> (in centimeters per second)
agent_get_position_in_group                      = 3333  # (agent_get_position_in_group, <position_register>, <agent_no>),
                                                         # Stores <agent_no>'s position in group into <position_register> (requires WSE2)
agent_get_current_ai_mesh_face_group             = 3334  # (agent_get_current_ai_mesh_face_group, <destination>, <agent_no>),
                                                         # Stores <agent_no>'s current ai mesh face group into <destination> (requires WSE2)
agent_set_time_speed_multiplier                  = 3335  # (agent_set_time_speed_multiplier, <agent_no>, <value_fixed_point>),
                                                         # Sets <agent_no>'s time speed multiplier to <value_fixed_point> (requires WSE2)
agent_get_time_speed_multiplier                  = 3336  # (agent_get_time_speed_multiplier, <destination_fixed_point>, <agent_no>),
                                                         # Stores <agent_no>'s time speed multiplier into <destination_fixed_point> (requires WSE2)
agent_kick                                       = 3337  # (agent_kick, <agent_no>),
                                                         # AI <agent_no> performs kick (requires WSE2)
agent_set_dropped_items_prune_time               = 3338  # (agent_set_dropped_items_prune_time, <agent_no>, <value>),
                                                         # Sets <agent_no>'s dropped items prune time to <value> (requires WSE2)
agent_set_missile_items_prune_time               = 3339  # (agent_set_missile_items_prune_time, <agent_no>, <value>),
                                                         # Sets <agent_no>'s missile items prune time to <value> (requires WSE2)
agent_set_action_speed_modifier                  = 3340  # (agent_set_action_speed_modifier, <agent_no>, <value>),
                                                         # Sets <agent_no>'s action speed modifier to <value> (requires WSE2)
agent_get_action_speed_modifier                  = 3341  # (agent_get_action_speed_modifier, <destination>, <agent_no>),
                                                         # Stores <agent_no>'s action speed modifier into <destination> (requires WSE2)
agent_set_left_hand_weapon_collision             = 3342  # (agent_set_left_hand_weapon_collision, <agent_no>, <value>),
                                                         # Enables or disables <agent_no>'s left hand weapon collision (requires WSE2)
agent_ai_set_can_weapon_switch                   = 3343  # (agent_ai_set_can_weapon_switch, <agent_no>, <value>),
                                                         # Enables or disables <agent_no>'s weapon switching for ai (requires WSE2)
...
multiplayer_send_chat_message_to_player      = 3400  # (multiplayer_send_chat_message_to_player, <player_no>, <sender_player_no>, <text>, [<type>]),
                                                     # Sends <text> to <player_no> as a (native compatible) chat message by <sender_player_no>. Works only on servers. [<type>]: 0 = chat, 1 = team chat
multiplayer_send_composite_message_to_player = 3401  # (multiplayer_send_composite_message_to_player, <player_no>, <message_type>, <message_register>),
                                                     # Sends <message_register> with <message_type> to <player_no> (requires, for WSE: network_compatible = 0 in wse_settings.ini, for WSE2: bBreakWarbandCompatibility=true in rgl_config.ini)
multiplayer_send_composite_message_to_server = 3402  # (multiplayer_send_composite_message_to_server, <message_type>, <message_register>),
                                                     # Sends <message_register> with <message_type> to the server (requires, for WSE: network_compatible = 0 in wse_settings.ini, for WSE2: bBreakWarbandCompatibility=true in rgl_config.ini)
multiplayer_get_cur_profile                  = 3403  # (multiplayer_get_cur_profile, <destination>),
                                                     # Stores the current multiplayer profile into <destination>
multiplayer_get_num_profiles                 = 3404  # (multiplayer_get_num_profiles, <destination>),
                                                     # Stores the number of multiplayer profiles into <destination>
multiplayer_message_init                     = 3405  # (multiplayer_message_init, <message_register>),
                                                     # Initializes (empties) <message_register> (requires, for WSE: network_compatible = 0 in wse_settings.ini, for WSE2: bBreakWarbandCompatibility=true in rgl_config.ini)
multiplayer_message_put_string               = 3406  # (multiplayer_message_put_string, <message_register>, <string>),
                                                     # Puts <string> into <message_register> (requires, for WSE: network_compatible = 0 in wse_settings.ini, for WSE2: bBreakWarbandCompatibility=true in rgl_config.ini)
...
server_set_password_admin      = 3500  # (server_set_password_admin, <password>),
                                       # Sets <password> as server administrator password
server_set_password_private    = 3501  # (server_set_password_private, <password>),
                                       # Sets <password> as server private player password
server_map_rotation_get_count  = 3502  # (server_map_rotation_get_count, <destination>),
                                       # Stores the number of maps in rotation into <destination>
server_map_rotation_get_index  = 3503  # (server_map_rotation_get_index, <destination>),
                                       # Stores the current map rotation index into <destination>
server_map_rotation_set_index  = 3504  # (server_map_rotation_set_index, <index>),
                                       # Sets the current rotation index to <index>
server_map_rotation_get_map    = 3505  # (server_map_rotation_get_map, <destination>, <index>),
                                       # Stores the map at <index> into <destination>
server_map_rotation_add_map    = 3506  # (server_map_rotation_add_map, <site_no>, [<index>]),
                                       # Adds <site_no> to the map rotation at [<index>]
server_map_rotation_remove_map = 3507  # (server_map_rotation_remove_map, [<index>]),
                                       # Removes the map at [<index>] from the map rotation (does not work when only one left)
server_get_horse_friendly_fire = 3508  # (server_get_horse_friendly_fire, <destination>),
                                       # Stores horse friendly fire status into <destination> (requires, for WSE: network_compatible = 0 in wse_settings.ini, for WSE2: bBreakWarbandCompatibility=true in rgl_config.ini)
server_set_horse_friendly_fire = 3509  # (server_set_horse_friendly_fire, <value>),
                                       # Enables or disables horse friendly fire (requires, for WSE: network_compatible = 0 in wse_settings.ini, for WSE2: bBreakWarbandCompatibility=true in rgl_config.ini)
server_get_show_crosshair      = 3510  # (server_get_show_crosshair, <destination>),
                                       # Stores crosshair visibility status into <destination> (requires, for WSE: network_compatible = 0 in wse_settings.ini, for WSE2: bBreakWarbandCompatibility=true in rgl_config.ini)
server_set_show_crosshair      = 3511  # (server_set_show_crosshair, <value>),
                                       # Enables or disables the crosshair (requires, for WSE: network_compatible = 0 in wse_settings.ini, for WSE2: bBreakWarbandCompatibility=true in rgl_config.ini)
get_server_option_at_connect   = 3512  # (get_server_option_at_connect, <destination>, [<index>]),
                                       # Stores option [<index>] into <destination>
server_set_password_rcon       = 3513  # (server_set_password_rcon, <password>),
                                       # Sets <password> as server RCON password
execute_server_console_command = 3514  # (execute_server_console_command, <string_register>, <command>),
                                       # Executes dedicated server console command <command> and stores result string into <string_register>
add_anonymous_player           = 3515  # (add_anonymous_player, <unique_id>, <name>),
                                       # Sets <name> pseudonym for multiplayer player <unique_id> (requires WSE2)
remove_anonymous_player        = 3516  # (remove_anonymous_player, <unique_id>),
                                       # Removes pseudonym for multiplayer player <unique_id> (requires WSE2)
...
set_show_use_tooltip                 = 3601  # (set_show_use_tooltip, <tooltip_type>, [<value>]),
...
particle_system_remove               = 3603  # (particle_system_remove, [<particle_system_no>]),
...
get_spectated_agent_no               = 3604  # (get_spectated_agent_no, <destination>),
                                             # Stores spectated agent no into <destination>
prop_instance_set_forced_lod         = 3605  # (prop_instance_set_forced_lod, <prop_instance_no>, <lod_level>),
                                             # Forces <prop_instance_no>'s LOD level to <lod_level> (0 = auto)
prop_instance_set_variation_id       = 3606  # (prop_instance_set_variation_id, <prop_instance_no>, <value>),
                                             # Sets <prop_instance_no>'s variation id to <value>
prop_instance_set_variation_id_2     = 3607  # (prop_instance_set_variation_id_2, <prop_instance_no>, <value>),
                                             # Sets <prop_instance_no>'s variation id 2 to <value>
stop_time                            = 3608  # (stop_time, <value>),
...
get_water_level                      = 3610  # (get_water_level, <destination_fixed_point>),
                                             # Stores the water level into <destination_fixed_point>
missile_remove_on_hit                = 3611  # (missile_remove_on_hit),
                                             # Causes a missile item not to spawn on hit (can be only used inside ti_on_missile_hit)
missile_is_valid                     = 3612  # (missile_is_valid, <missile_no>),
                                             # Fails if <missile_no> is not valid
missile_get_cur_position             = 3613  # (missile_get_cur_position, <position_register>, <missile_no>),
                                             # Stores <missile_no>'s current position into <position_register>
set_prop_collision_threshold         = 3614  # (set_prop_collision_threshold, <attack_direction>, <low_boundary>, <high_boundary>),
...
get_camera_position                  = 3615  # (get_camera_position, <position_register>),
                                             # Stores camera position and rotation into <position_register>.
prop_instance_remove_particle_system = 3616  # (prop_instance_remove_particle_system, <prop_instance_no>, [<particle_system_no>]),
                                             # Removes [<particle_system_no>] (all particle systems if not set or -1) from <prop_instance_no>
prop_instance_remove_light           = 3617  # (prop_instance_remove_light, <prop_instance_no>),
                                             # Removes light from <prop_instance_no>
prop_instance_get_sound_progress     = 3618  # (prop_instance_get_sound_progress, <destination>, <scene_prop_id>),
                                             # Stores <scene_prop_id>'s sound_progress into <destination>. Returned value can be between 0-100, or -1 if nothing is being played. (requires WSE2)
set_horse_friendly_fire              = 3619  # (set_horse_friendly_fire, <value>),
...
troop_get_skill_points       = 3700  # (troop_get_skill_points, <destination>, <troop_no>),
                                     # Stores <troop_no>'s unused skill points into <destination>
troop_set_skill_points       = 3701  # (troop_set_skill_points, <troop_no>, <value>),
                                     # Sets <troop_no>'s unused skill points to <value>
troop_get_attribute_points   = 3702  # (troop_get_attribute_points, <destination>, <troop_no>),
                                     # Stores <troop_no>'s unused attribute points into <destination>
troop_set_attribute_points   = 3703  # (troop_set_attribute_points, <troop_no>, <value>),
                                     # Sets <troop_no>'s unused attribute points to <value>
troop_get_proficiency_points = 3704  # (troop_get_proficiency_points, <destination>, <troop_no>),
                                     # Stores <troop_no>'s unused proficiency points into <destination>
troop_set_proficiency_points = 3705  # (troop_set_proficiency_points, <troop_no>, <value>),
                                     # Sets <troop_no>'s unused proficiency points to <value>
troop_has_flag               = 3706  # (troop_has_flag, <troop_no>, <flag>),
                                     # Fails if <troop_no> doesn't have <flag>
troop_set_skill              = 3707  # (troop_set_skill, <troop_no>, <skill_no>, <value>),
                                     # Sets <troop_no>'s <skill_no> to <value>
troop_set_attribute          = 3708  # (troop_set_attribute, <troop_no>, <attribute>, <value>),
                                     # Sets <troop_no>'s <attribute> to <value>
troop_set_proficiency        = 3709  # (troop_set_proficiency, <troop_no>, <proficiency>, <value>),
                                     # Sets <troop_no>'s <proficiency> to <value>

item_set_thrust_damage          = 3800  # (item_set_thrust_damage, <item_kind_no>, <value>),
                                        # Sets <item_kind_no>'s thrust damage to <value>
item_set_thrust_damage_type     = 3801  # (item_set_thrust_damage_type, <item_kind_no>, <value>),
                                        # Sets <item_kind_no>'s thrust damage type to <value>
item_set_swing_damage           = 3802  # (item_set_swing_damage, <item_kind_no>, <value>),
                                        # Sets <item_kind_no>'s thrust damage to <value>
item_set_swing_damage_type      = 3803  # (item_set_swing_damage_type, <item_kind_no>, <value>),
                                        # Sets <item_kind_no>'s thrust damage type to <value>
item_set_head_armor             = 3804  # (item_set_head_armor, <item_kind_no>, <value>),
                                        # Sets <item_kind_no>'s head armor to <value>
item_set_body_armor             = 3805  # (item_set_body_armor, <item_kind_no>, <value>),
                                        # Sets <item_kind_no>'s body armor to <value>
item_set_leg_armor              = 3806  # (item_set_leg_armor, <item_kind_no>, <value>),
                                        # Sets <item_kind_no>'s leg armor to <value>
item_set_speed_rating           = 3807  # (item_set_speed_rating, <item_kind_no>, <value>),
                                        # Sets <item_kind_no>'s speed rating to <value>
item_set_missile_speed          = 3808  # (item_set_missile_speed, <item_kind_no>, <value>),
                                        # Sets <item_kind_no>'s missile speed to <value>
item_set_horse_blood_particles  = 3809  # (item_set_horse_blood_particles, <item_kind_no>, <particle_1_no>, <particle_2_no>),
                                        # Sets <item_kind_no>'s horse blood <particle_1_no> and <particle_2_no>
item_set_horse_blood_color      = 3810  # (item_set_horse_blood_color, <item_kind_no>, <color>),
                                        # Sets <item_kind_no>'s horse blood <color> (requires WSE2)
...
party_stack_get_num_upgradeable = 3901  # (party_stack_get_num_upgradeable, <destination>, <party_no>, <party_stack_no>),
                                        # Stores the amount of upgradeable troops in <party_no>'s <party_stack_no> into <destination>
party_has_flag                  = 3902  # (party_has_flag, <party_no>, <flag>),
                                        # Fails if <party_no> doesn't have <flag>
party_heal_members              = 3903  # (party_heal_members, <party_no>, <troop_no>, <number>),
                                        # Heals <number> <troop_no> of <party_no>
party_switch_stacks             = 3904  # (party_switch_stacks, <party_no>, <party_stack_no_1>, <party_stack_no_2>),
                                        # Switches <party_no>'s <party_stack_no_1> and <party_stack_no_2>
party_stack_upgrade             = 3905  # (party_stack_upgrade, <party_no>, <party_stack_no>, <amount>, <upgrade_path>),
                                        # Upgrades <party_no>'s <party_stack_no>'s <amount> of troops (<upgrade_path> can be 0 or 1) (requires WSE2)
party_stack_set_num_upgradeable = 3906  # (party_stack_set_num_upgradeable, <party_no>, <party_stack_no>, <value>),
                                        # Sets <party_no>'s <party_stack_no>'s amount of upgradeable troops to <value>
party_get_banner_icon           = 3907  # (party_get_banner_icon, <destination>, <party_no>),
                                        # Stores <party_no>'s banner icon into <destination>
party_get_extra_icon            = 3908  # (party_get_extra_icon, <destination>, <party_no>),
                                        # Stores <party_no>'s extra icon into <destination>
party_get_player_id             = 3909  # (party_get_player_id, <destination>, <party_no>),
                                        # Stores <party_no>'s player reference into <destination>. For multiplayer campaign mode (requires WSE2)
party_is_non_player             = 3910  # (party_is_non_player, <party_no>),
                                        # Fails if <party_no> is player. For multiplayer campaign mode (requires WSE2)

position_get_vector_to_position = 4100  # (position_get_vector_to_position, <destination_fixed_point>, <dest_position_register>, <position_register_1>, <position_register_2>),
                                        # Stores the vector from <position_register_1> to <position_register_2> into <dest_position_register> and its length into <destination_fixed_point>
position_align_to_ground        = 4101  # (position_align_to_ground, <position_register>, [<point_up>], [<set_z_to_ground_level>]),
                                        # Aligns <position_register> to the ground (or to the ground normal if [<point_up>] is set)
position_get_length             = 4102  # (position_get_length, <destination_fixed_point>, <position_register>),
                                        # Stores <position_register> length into <destination_fixed_point>
get_dot_product_of_positions    = 4103  # (get_dot_product_of_positions, <destination_fixed_point>, <position_register_1>, <position_register_2>),
                                        # Stores <position_register_1> and <position_register_2> dot product into <destination_fixed_point>
get_cross_product_of_positions  = 4104  # (get_cross_product_of_positions, <dest_position_register>, <position_register_1>, <position_register_2>),
                                        # Stores <position_register_1> and <position_register_2> cross product into <dest_position_register>

str_equals                                = 4200  # (str_equals, <string_1>, <string_2>, [<case_insensitive>]),
                                                  # Fails if <string_1> is not equal to <string_2>
str_contains                              = 4201  # (str_contains, <string_1>, <string_2>, [<case_insensitive>]),
                                                  # Fails if <string_1> doesn't contain <string_2>
str_starts_with                           = 4202  # (str_starts_with, <string_1>, <string_2>, [<case_insensitive>]),
                                                  # Fails if <string_1> doesn't start with <string_2>
str_ends_with                             = 4203  # (str_ends_with, <string_1>, <string_2>, [<case_insensitive>]),
                                                  # Fails if <string_1> doesn't end with <string_2>
str_is_alpha                              = 4204  # (str_is_alpha, <string_1>, [<index>]),
                                                  # Fails if character [<index>] of <string_1> isn't alphabetic. If [<index>] is not defined or is -1, the entire string is checked
str_is_digit                              = 4205  # (str_is_digit, <string_1>, [<index>]),
                                                  # Fails if character [<index>] of <string_1> isn't a digit. If [<index>] is not defined or is -1, the entire string is checked
str_is_whitespace                         = 4206  # (str_is_whitespace, <string_1>, [<index>]),
                                                  # Fails if character [<index>] of <string_1> isn't whitespace. If [<index>] is not defined or is -1, the entire string is checked
...
str_store_lower                           = 4215  # (str_store_lower, <string_register>, <string_1>),
                                                  # Stores the lowercase version of <string_1> into <string_register>
...
str_store_replace_underscores_with_spaces = 4224  # (str_store_replace_underscores_with_spaces, <string_register>, <string_1>),
                                                  # Stores <string_1> into <string_register>, replacing all underscores with spaces
str_store_multiplayer_profile_name        = 4225  # (str_store_multiplayer_profile_name, <string_register>, <profile_no>),
                                                  # Stores <profile_no>'s name into <string_register>
str_store_module_setting                  = 4226  # (str_store_module_setting, <string_register>, <setting>),
                                                  # Stores the string value (empty if not found) of <setting> in module.ini into <string_register>
str_store_server_password_admin           = 4227  # (str_store_server_password_admin, <string_register>),
                                                  # Stores the server administrator password into <string_register>
str_store_server_password_private         = 4228  # (str_store_server_password_private, <string_register>),
                                                  # Stores the server private player password into <string_register>
str_store_overlay_text                    = 4229  # (str_store_overlay_text, <string_register>, <overlay_no>),
                                                  # Stores <overlay_no>'s text into <string_register>
str_store_player_ip                       = 4230  # (str_store_player_ip, <string_register>, <player_no>),
                                                  # Stores <player_no>'s IP address into <string_register> (works only on servers)
str_store_game_variable                   = 4231  # (str_store_game_variable, <string_register>, <variable>),
                                                  # Stores the string value (empty if not found) of <variable> in game_variables.txt into <string_register>
str_store_skill_name                      = 4232  # (str_store_skill_name, <string_register>, <skill_no>),
                                                  # Stores the name of <skill_no> into <string_register>
str_store_float                           = 4233  # (str_store_float, <string_register>, <fp_register>, [<precision>]),
                                                  # Stores the string representation of <fp_register> into <string_register> showing [<precision>] decimal digits at most
str_sanitize                              = 4234  # (str_sanitize, <string_register>),
                                                  # Removes invalid characters from <string_register>
str_store_item_id                         = 4235  # (str_store_item_id, <string_register>, <item_no>),
                                                  # Stores the id of <item_no> into <string_register>
str_is_integer                            = 4236  # (str_is_integer, <string_1>),
                                                  # Fails if <string_1> isn't a valid integer
str_store_multiplayer_profile_face_keys   = 4237  # (str_store_multiplayer_profile_face_keys, <string_register>, <profile_no>),
                                                  # Stores <profile_no>'s face keys into <string_register>
str_store_server_password_rcon            = 4238  # (str_store_server_password_rcon, <string_register>),
                                                  # Stores the server RCON password into <string_register>
str_store_item_mesh_name                  = 4239  # (str_store_item_mesh_name, <string_register>, <item_no>),
                                                  # Stores the mesh name of <item_no> into <string_register>
str_regex_match                           = 4240  # (str_regex_match, <string_1>, <string_regex>),
                                                  # Fails if <string_1> does not match <string_regex>
...
options_get_verbose_casualties  = 4300  # (options_get_verbose_casualties, <destination>),
                                        # Stores verbose casualties enabled/disabled into <destination>
options_set_verbose_casualties  = 4301  # (options_set_verbose_casualties, <value>),
                                        # Enables or disables verbose casualties
options_get_cheat_mode          = 4302  # (options_get_cheat_mode, <destination>),
                                        # Stores cheat mode enabled/disabled into <destination>
options_set_cheat_mode          = 4303  # (options_set_cheat_mode, <value>),
                                        # Enables or disables cheat mode
options_get_realistic_headshots = 4304  # (options_get_realistic_headshots, <destination>),
                                        # Stores "realistic" headshots enabled/disabled into <destination>
options_set_realistic_headshots = 4305  # (options_set_realistic_headshots, <value>),
                                        # Enables or disables "realistic" headshots

fld       = 4400  # (fld, <fp_register>, <value_fixed_point>),
                  # Loads <value_fixed_point> into <fp_register>
fld_str   = 4401  # (fld_str, <fp_register>, <string>),
                  # Parses <string> and loads it into <fp_register>
fld_pos_x = 4402  # (fld_pos_x, <fp_register>, <position_register_no>),
                  # Loads the x component of <position_register_no> into <fp_register>
fld_pos_y = 4403  # (fld_pos_y, <fp_register>, <position_register_no>),
                  # Loads the y component of <position_register_no> into <fp_register>
fld_pos_z = 4404  # (fld_pos_z, <fp_register>, <position_register_no>),
                  # Loads the z component of <position_register_no> into <fp_register>
fst       = 4405  # (fst, <destination_fixed_point>, <fp_register>),
                  # Stores <fp_register> into <destination_fixed_point>
fcpy      = 4406  # (fcpy, <fp_register_1>, <fp_register_2>),
                  # Copies <fp_register_2> into <fp_register_1>
feq       = 4407  # (feq, <destination_fp_register>, <fp_register_1>, <fp_register_2>),
                  # Fails if <fp_register_1> isn't approximately equal to <fp_register_2>
fgt       = 4408  # (fgt, <destination_fp_register>, <fp_register_1>, <fp_register_2>),
                  # Fails if <fp_register_1> isn't greater than <fp_register_2>
flt       = 4409  # (flt, <destination_fp_register>, <fp_register_1>, <fp_register_2>),
                  # Fails if <fp_register_1> isn't less than <fp_register_2>
fge       = 4410  # (fge, <destination_fp_register>, <fp_register_1>, <fp_register_2>),
                  # Fails if <fp_register_1> isn't greater or approximately equal to <fp_register_2>
fle       = 4411  # (fle, <destination_fp_register>, <fp_register_1>, <fp_register_2>),
                  # Fails if <fp_register_1> isn't less or approximately equal to <fp_register_2>
fsub      = 4412  # (fsub, <destination_fp_register>, <fp_register_1>, <fp_register_2>),
                  # Subtracts <fp_register_2> from <fp_register_1> and stores the result into <destination_fp_register>
fmul      = 4413  # (fmul, <destination_fp_register>, <fp_register_1>, <fp_register_2>),
                  # Multiplies <fp_register_1> by <fp_register_2> and stores the result into <destination_fp_register>
fdiv      = 4414  # (fdiv, <destination_fp_register>, <fp_register_1>, <fp_register_2>),
                  # Divides <fp_register_1> by <fp_register_2> and stores the result into <destination_fp_register>
fmin      = 4415  # (fmin, <destination_fp_register>, <fp_register_1>, <fp_register_2>),
                  # Stores the smaller value between <fp_register_1> and <fp_register_2> into <destination_fp_register>
fmax      = 4416  # (fmax, <destination_fp_register>, <fp_register_1>, <fp_register_2>),
                  # Stores the larger value between <fp_register_1> and <fp_register_2> into <destination_fp_register>
fclamp    = 4417  # (fclamp, <destination_fp_register>, <fp_register_1>, <fp_register_2>, <fp_register_3>),
                  # Clamps <fp_register_1> between <fp_register_2> and <fp_register_3> and stores the result into <destination_fp_register>
fsqrt     = 4418  # (fsqrt, <destination_fp_register>, <fp_register>),
                  # Stores the square root of <fp_register> into <destination_fp_register>
fabs      = 4419  # (fabs, <destination_fp_register>, <fp_register>),
                  # Stores the absolute value of <fp_register> into <destination_fp_register>
fceil     = 4420  # (fceil, <destination_fp_register>, <fp_register>),
                  # Stores the ceiling of <fp_register> into <destination_fp_register>
ffloor    = 4421  # (ffloor, <destination_fp_register>, <fp_register>),
                  # Stores the floor of <fp_register> into <destination_fp_register>
fexp      = 4422  # (fexp, <destination_fp_register>, <fp_register>),
                  # Stores e raised to the power of <fp_register> into <destination_fp_register>
fpow      = 4423  # (fpow, <destination_fp_register>, <fp_register_1>, <fp_register_2>),
                  # Stores <fp_register_1> raised to the power of <fp_register_2> into <destination_fp_register>
fln       = 4424  # (fln, <destination_fp_register>, <fp_register>),
                  # Stores the natural logarithm of <fp_register> into <destination_fp_register>
flog      = 4425  # (flog, <destination_fp_register>, <fp_register>, [<base>]),
                  # Stores the base-[<base>] (default: base-10) logarithm of <fp_register> into <destination_fp_register>
fmod      = 4426  # (fmod, <destination_fp_register>, <fp_register_1>, <fp_register_2>),
                  # Stores the remainder of <fp_register_1>/<fp_register_2> into <destination_fp_register>
fsin      = 4427  # (fsin, <destination_fp_register>, <fp_register>),
                  # Stores the sine of <fp_register> into <destination_fp_register>
fcos      = 4428  # (fcos, <destination_fp_register>, <fp_register>),
                  # Stores the cosine of <fp_register> into <destination_fp_register>
ftan      = 4429  # (ftan, <destination_fp_register>, <fp_register>),
                  # Stores the tangent of <fp_register> into <destination_fp_register>
fsinh     = 4430  # (fsinh, <destination_fp_register>, <fp_register>),
                  # Stores the hyperbolic sine of <fp_register> into <destination_fp_register>
fcosh     = 4431  # (fcosh, <destination_fp_register>, <fp_register>),
                  # Stores the hyperbolic cosine of <fp_register> into <destination_fp_register>
ftanh     = 4432  # (ftanh, <destination_fp_register>, <fp_register>),
                  # Stores the hyperbolic tangent of <fp_register> into <destination_fp_register>
fasin     = 4433  # (fasin, <destination_fp_register>, <fp_register>),
                  # Stores the arc sine of <fp_register> into <destination_fp_register>
facos     = 4434  # (facos, <destination_fp_register>, <fp_register>),
                  # Stores the arc cosine of <fp_register> into <destination_fp_register>
fatan     = 4435  # (fatan, <destination_fp_register>, <fp_register>),
                  # Stores the arc tangent of <fp_register> into <destination_fp_register>
fatan2    = 4436  # (fatan2, <destination_fp_register>, <fp_register_1>, <fp_register_2>),
                  # Stores the arc cosine of <fp_register_1>/<fp_register_2> into <destination_fp_register>
feval     = 4437  # (feval, <expression_string>),
                  # Evaluates <expression_string>. See EVAL_README.txt in WSESDK for more information
fadd      = 4438  # (fadd, <destination_fp_register>, <fp_register_1>, <fp_register_2>),
                  # Adds <fp_register_2> to <fp_register_1> and stores the result into <destination_fp_register>

scene_set_flags         = 4500  # (scene_set_flags, <scene_no>, <flags>),
                                # Sets <scene_no>'s flags to <flags>
scene_set_water_level   = 4501  # (scene_set_water_level, <scene_no>, <water_level_fixed_point>),
                                # Sets <scene_no>'s water level to <water_level_fixed_point>
scene_set_bounds        = 4502  # (scene_set_bounds, <scene_no>, <min_x_fixed_point>, <min_y_fixed_point>, <max_x_fixed_point>, <max_y_fixed_point>),
                                # Sets <scene_no>'s bounds to (<min_x_fixed_point>, <min_y_fixed_point>) and (<max_x_fixed_point>, <max_y_fixed_point>)
scene_set_outer_terrain = 4503  # (scene_set_outer_terrain, <scene_no>, <outer_terrain_mesh_name>),
                                # Sets <scene_no>'s outer terrain to <outer_terrain_mesh_name>
scene_set_terrain_seed  = 4504  # (scene_set_terrain_seed, <scene_no>, <value>),
                                # Sets <scene_no>'s terrain generator terrain seed to <value>
scene_set_river_seed    = 4505  # (scene_set_river_seed, <scene_no>, <value>),
                                # Sets <scene_no>'s terrain generator river seed to <value>
scene_set_flora_seed    = 4506  # (scene_set_flora_seed, <scene_no>, <value>),
                                # Sets <scene_no>'s terrain generator flora seed to <value>
scene_set_deep_water    = 4507  # (scene_set_deep_water, <scene_no>, <value>),
                                # Enables or disables terrain generator deep water for <scene_no>
scene_set_place_river   = 4508  # (scene_set_place_river, <scene_no>, <value>),
                                # Enables or disables terrain generator river placement for <scene_no>
scene_set_disable_grass = 4509  # (scene_set_disable_grass, <scene_no>, <value>),
                                # Enables or disables terrain generator grass placement for <scene_no>
scene_set_valley_size   = 4510  # (scene_set_valley_size, <scene_no>, <value>),
                                # Sets <scene_no>'s terrain generator valley size to <value> (0-127)
scene_set_hill_height   = 4511  # (scene_set_hill_height, <scene_no>, <value>),
                                # Sets <scene_no>'s terrain generator hill height to <value> (0-127)
scene_set_ruggedness    = 4512  # (scene_set_ruggedness, <scene_no>, <value>),
                                # Sets <scene_no>'s terrain generator ruggedness to <value> (0-127)
scene_set_vegetation    = 4513  # (scene_set_vegetation, <scene_no>, <value>),
                                # Sets <scene_no>'s terrain generator vegetation to <value> (0-127)
scene_set_size          = 4514  # (scene_set_size, <scene_no>, <x>, <y>),
                                # Sets <scene_no>'s terrain generator map size to (<x>, <y>) (0-1023)
scene_set_region_type   = 4515  # (scene_set_region_type, <scene_no>, <value>),
                                # Sets <scene_no>'s terrain generator region type to <value> (0-15)
scene_set_region_detail = 4516  # (scene_set_region_detail, <scene_no>, <value>),
                                # Sets <scene_no>'s terrain generator region detail to <value> (0-3)

edit_mode_get_num_selected_prop_instances = 4600  # (edit_mode_get_num_selected_prop_instances, <destination>),
                                                  # Stores the number of selected prop instances into <destination>
edit_mode_get_selected_prop_instance      = 4601  # (edit_mode_get_selected_prop_instance, <destination>, <index>),
                                                  # Stores the <index>th selected prop instance into instance no into <destination>
edit_mode_select_prop_instance            = 4602  # (edit_mode_select_prop_instance, <prop_instance_no>),
                                                  # Stores the <1>th selected prop instance into instance no into <prop_instance_no>
edit_mode_deselect_prop_instance          = 4603  # (edit_mode_deselect_prop_instance, <prop_instance_no>),
                                                  # Stores the <1>th selected prop instance into instance no into <prop_instance_no>
edit_mode_get_highlighted_prop_instance   = 4604  # (edit_mode_get_highlighted_prop_instance, <destination>),
                                                  # Stores the highlighted prop instance into <destination>
edit_mode_set_highlighted_prop_instance   = 4605  # (edit_mode_set_highlighted_prop_instance, [<prop_instance_no>]),
                                                  # Stores the <1>th selected prop instance into instance no into [<prop_instance_no>]
edit_mode_set_enabled                     = 4606  # (edit_mode_set_enabled, <value>),
                                                  # Enables or disables edit mode
edit_mode_in_edit_objects_mode            = 4607  # (edit_mode_in_edit_objects_mode),
                                                  # Fails if the game is not in edit objects mode

update_material   = 4700  # (update_material, <material_name>, <new_material_name>),
                          # Updates <material_name> with <new_material_name>
reload_item_kinds = 4701  # (reload_item_kinds, [<change_file>], [<file_name>]),
                          # Reload item kinds. If [<change_file>] sets, then [<file_name>] loaded instead default file (requires WSE2)
reload_troops     = 4702  # (reload_troops, [<change_file>], [<file_name>]),
                          # Reload troops. If [<change_file>] sets, then [<file_name>] loaded instead default file (requires WSE2)
reload_parties    = 4703  # (reload_parties, [<change_file>], [<file_name>]),
...
overlay_get_val         = 4900  # (overlay_get_val, <destination>, <overlay_no>),
                                # Stores <overlay_no>'s value into <destination>
presentation_activate   = 4901  # (presentation_activate, <presentation_no>),
                                # Activates <presentation_no>. Fails if <presentation_no> is not running
...
overlay_get_scroll_pos  = 4903  # (overlay_get_scroll_pos, <destination_fixed_point>, <overlay_no>, [<horizontal>]),
                                # Stores <overlay_no>'s scroll pos into <destination_fixed_point>
overlay_set_scroll_pos  = 4904  # (overlay_set_scroll_pos, <overlay_no>, <value_fixed_point>, [<horizontal>]),
                                # Sets <overlay_no>'s scroll pos <value_fixed_point>
overlay_enable          = 4905  # (overlay_enable, <overlay_no>, <enable_or_disable>),
                                # Sets <overlay_no>'s <enable_or_disable>
overlay_item_set_text   = 4906  # (overlay_item_set_text, <overlay_no>, <item_no>, <text>),
...
array_copy          = 5002  # (array_copy, <destination>, <source arrayID>),
                            # Copys array with <source arrayID> and stores the new array id into <destination>.
array_save_file     = 5003  # (array_save_file, <arrayID>, <file>),
                            # Saves <arrayID> into a file. For security reasons, <file> is just a name, not a full path, and will be stored into a WSE managed directory.
array_load_file     = 5004  # (array_load_file, <destination>, <file>),
                            # Loads <file> as an array and stores the newly created array's ID into <destination>.
...
array_get_dim_size  = 5012  # (array_get_dim_size, <destination>, <arrayID>, <dimIndex>),
                            # Gets the size of the dimension with <dimIndex> of the array with <arrayID> and stores it into <destination>.
array_get_dim_count = 5013  # (array_get_dim_count, <destination>, <arrayID>),
                            # Gets the the amount of dimensions of the array with <arrayID> and stores it into <destination>.
...
lua_insert          = 5102  # (lua_insert, <index>),
                            # Moves the stacks top element into <index>, shifting up the elements above <index> to open space.
lua_remove          = 5103  # (lua_remove, <index>),
                            # Removes the element at the given <index>, shifting down the elements above <index> to fill the gap.
lua_pop             = 5104  # (lua_pop, <value>),
                            # Pops <value> values from the lua stack.
lua_to_int          = 5105  # (lua_to_int, <destination>, <index>),
                            # Retrieves the value at <index> from the lua stack and stores it in <destination>
lua_to_str          = 5106  # (lua_to_str, <string_register>, <index>),
                            # Retrieves the string at <index> from the lua stack and stores it in <string_register>
lua_to_pos          = 5107  # (lua_to_pos, <pos_register>, <index>),
                            # Retrieves the position at <index> from the lua stack and stores it in <pos_register>
lua_push_int        = 5108  # (lua_push_int, <value>),
                            # Pushes <value> onto the lua stack.
lua_push_str        = 5109  # (lua_push_str, <string_1>),
                            # Pushes <string_1> onto the lua stack.
...
agent_set_attached_scene_prop               = 1757  # (agent_set_attached_scene_prop, <agent_no>, <prop_instance_no>, [<attached_prop_index>], [<bone_no>], [<use_bone_rotation>]),
                                                    # Attaches the specified <prop_instance_no> to the <agent_no>. ([<attached_prop_index>]: 0-3) (requires WSE2)
agent_set_attached_scene_prop_x             = 1758  # (agent_set_attached_scene_prop_x, <agent_no>, <value>, [<attached_prop_index>]),
                                                    # Offsets the position of the attached scene prop in relation to <agent_no>, in centimeters, along the X axis (left/right). ([<attached_prop_index>]: 0-3) (requires WSE2)
agent_set_attached_scene_prop_z             = 1759  # (agent_set_attached_scene_prop_z, <agent_no>, <value>, [<attached_prop_index>]),
                                                    # Offsets the position of the attached scene prop in relation to <agent_no>, in centimeters, along the Z axis (down/up). ([<attached_prop_index>]: 0-3) (requires WSE2)
entry_point_get_position                    = 1780  # (entry_point_get_position, <position_register>, <entry_no>, [<shifted>]),
                                                    # Stores <entry_no>'s position into <position_register>. If [<shifted>] is non-zero stores shifted position (requires WSE2)
...
str_store_module_setting = 4226  # (str_store_module_setting, <string_register>, <setting>, <section>),
                                 # Stores the string value (empty if not found) of <section>'s <setting> in rgl_config.ini into <string_register> (requires WSE2)
...
lhs_operations = [ # Left hand side operations make an assignment to a variable.
    ...
    agent_get_action_dir,                        agent_get_division,                                       agent_get_player_id,                             agent_set_reload_speed_modifier,                   
    agent_get_ammo_for_slot,                     agent_get_entry_no,                                       agent_get_rider,                                 agent_set_speed_modifier,                                           
    agent_get_ammo,                              agent_get_group,                                          agent_get_simple_behavior,                       agent_set_use_speed_modifier,                                                                   
    agent_get_animation,                         agent_get_horse,                                          agent_get_slot,                                  assign,                                              
